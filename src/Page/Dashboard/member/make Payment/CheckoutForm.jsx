...
const CheckoutForm = ({ paydata }) => {
  ...
  useEffect(() => {
    if (paydata?.totalPay) {
      axiosscure
        .post("create-payment-intent", { price: paydata.totalPay })
        .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (card === null) {
      ...
      toast.error("Payment card details are required.");
      return;
    }
    ...
    //   confim pay
    const { paymentIntent, error: confimerror } =
    ...
    if (confimerror) {
      ...
      toast.error(confimerror.message);
    } else {
